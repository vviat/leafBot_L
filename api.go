...
type (
	...
	//获取精华消息列表 20210726add
	EssenceMsgList struct {
		Sender_id     int    `json:"sender_id"`
		Sender_nick   string `json:"sender_nick"`
		Sender_time   int    `json:"sender_time"`
		Operator_id   int    `json:"operator_id"`
		Operator_nick string `json:"operator_nick"`
		Operator_time int    `json:"operator_time"`
		Message_id    int    `json:"message_id"`
	}
)

type (
	//获取精华消息列表返回值 20210726add
	responseEssenceMsgListJson struct {
		response
		Data []EssenceMsgList `json:"data"`
	}
...
//20210726获取精华消息函数实现
func (b *Bot) GetEssenceMsgList(groupId int) []EssenceMsgList {
	echo := uuid.NewV4().String()
	type param struct {
		GroupId int `json:"group_id"`
	}
	...
	b.Client.OutChan <- d
	data := getResponse(&response{}, echo)
	var response responseEssenceMsgListJson
	_ = json.Unmarshal(data, &response)
	content, _ := json.Marshal(d)
	log.Infoln(string(content) + "\n\t\t\t\t\t" + string(data))
	return response.Data
